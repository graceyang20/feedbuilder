...
- `/` : UI 프로토타입 (React)
- `/api/kiwoom/token` : 키움 mock API 토큰 발급 + 자동 갱신
  (`au10001` 규격 그대로 구현)
- `/api/kiwoom/index` : 코스피 지수 실시간 데이터
  (`ka20001 업종현재가요청` 규격 그대로 구현). 화면의 "지수" 카드에서
  **코스피 항목만** 이 API로 자동 교체됩니다 (S&P 500 / 달러 환율은
  별도 TR이 필요해서 아직 미연동 — 아래 표 참고).

## 지금까지 연결된 데이터 vs 아직 안 된 것

| 화면 | 항목 | 상태 | 근거 |
|---|---|---|---|
| 지수 카드 | 코스피 | ✅ 실시간 연결 | `ka20001` |
| 지수 카드 | S&P 500 | ⛔ 미연동 (고정값) | 별도 미국지수 TR 필요 |
| 지수 카드 | 달러 환율 | ⛔ 미연동 (고정값) | 별도 환전 TR 필요 |
| 그 외 모든 카드 (포트폴리오/관심종목/뉴스/랭킹/커뮤니티 등) | 전체 | ⛔ 미연동 (고정값) | 다음 단계에서 순차 연결 예정 |
